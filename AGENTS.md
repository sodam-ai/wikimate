...
> **찾기/물어보기**("내 볼트/위키에서 ~ 찾아줘·요약해줘")는 **읽기 전용** 흐름: 색인(노션)·원본(옵시디언)에서 찾되 **답하기 전에 원본 존재를 확인**하고, 끊긴 색인(원본 삭제)은 근거에서 제외 + 정직 보고(거짓 인용 금지). Claude Code는 `wikimate-query` 스킬.
...
## 안전 규칙 (척추)
- 쓰기는 **계획 보고(dry_run=true) → 사람 승인 → 실행**(기본). 사용자가 사전 승인("묻지 말고 바로")하면 **신규 생성은 자동 실행 가능**하되, **덮어쓰기·삭제·수정 등 비가역은 항상 개별 확인**(끌 수 없는 안전선) + 백업.
